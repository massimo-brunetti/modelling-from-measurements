...
%%% NN training on Lorenz augmented trajectory set (rho), then forecast %%%
...
for i = 1:length(R1)
    ...
    for ii = 1:mr  
        x0 = 30*(rand(3,1)-0.5);
        [~,y] = ode45(Lorenz,t,x0);
        yr = [y ones(length(y),1)*r]; % augmented state: x,y,z and rho
        input = [input; yr(1:end-1,:)];
        output = [output; yr(2:end,:)];               
...
for i =1:length(R2)  
    ...
    for ii = 1:2
        ...
        r_NN = floor(mean(y_NN(:,4))); % average rho, forecasted by NN
        
        figure(1);
        
        subplot(3,3,1+ii);
        plot(t,y(:,3),'r',t,y_NN(:,3),'b-.')
        title(['ODE sol. (rho = ' num2str(r) ') vs NN forecast (rho = ' num2str(r_NN) ')']);
        xlabel('Time');
        ylabel('Z');
        legend({'ODE','NN'});
        grid on;

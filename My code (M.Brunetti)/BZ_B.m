...
title(['Reshaped sol, projected/truncated up to ' num2str(r)]);
xlabel('Time')
ylabel('Amplitude')
grid on
...
subplot(2,2,4);
plot(t(1:ths),real(pu2_dmd(:,1:ths)),...
     t(ths:end),real(pu2_dmd(:,ths:end)),'-.');
...
title(['DMD reconstruction at time = ' num2str(0)...
...
title(['DMD reconstruction at time = ' num2str(floor(um/2)*dt)...
       ' based on ' num2str(r) ' modes']);
xlabel('X');
ylabel('Y');
...
title(['DMD forecast at time = ' num2str(um*dt)...
       ' based on ' num2str(r) ' modes']);
xlabel('X');
ylabel('Y')